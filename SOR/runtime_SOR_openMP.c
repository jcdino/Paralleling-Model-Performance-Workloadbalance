/****************************************************************************
gcc -O1 -fopenmp -std=gnu11 runtime_SOR_openMP.c -lpthread -lrt -lm -o runtime_SOR_openMP
*/
#define _GNU_SOURCE
#include <stdio.h>
#include <omp.h>
...
void SOR_openmp(arr_ptr v, int *iterations)
{
  ...
  while ((total_change/(double)(rowlen*rowlen)) > (double)TOL) {
    iters++;
    total_change = 0;
    #pragma omp parallel for schedule(runtime) num_threads(8) private(i, j, change) shared(rowlen, data) reduction(+:total_change)
